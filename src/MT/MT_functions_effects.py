...
def get_wts_t_mtag(t, Cov_g, Sigma_jL):
    ...
    for Sigma_j in Sigma_jL:
        A = np_inv(Cov_g - np.outer(omega_t, omega_t)/omega_tt + Sigma_j)
...
def get_gmm_wts(Omega_0, Omega_1, Sigma_jL):
    ...
    '''computes weights for posterior estimator of mean for each component, helper function for estimating beta_MT'''

    # beta | betahat, gamma=c ~ N(A_c Vinv betahat, A_c)  - posterior distribution of beta for gamma = c
    ...
    # calculate inverse for Omega_0 and Omega_1 if invertible, otherwise use pseudo-inverse
...
# for example, the numpy mvn objects and inverse var matrix (AV)
